...
def cv_train( X, Y, d,beta,s,c,regMin=1e-30, regMax=1.0, K=100 ,maxEigen=None ):
    BETAs = []
...
def run(seed, n, p, g,  k, sigX, sigY,we, model,d=0.05, simulate=False):
    ...
    beta_n,time=cv_train(X=X,Y=Y,d=d,beta=beta,s=sigX,c=sigY)
    print fileHead,"ok~"
    np.save(fileHead+'beta_lmm_select',beta_n)
